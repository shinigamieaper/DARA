"""Yoruba dictionary loader: kaikki.org Wiktextract of Yoruba Wiktionary.

CC BY-SA. Assigns every word to Standard Yoruba (dialect_id=1). See
...
SOURCE_TAG = SOURCE_TAGS["yoruba_dict"]

_URL = "https://kaikki.org/dictionary/Yoruba/kaikki.org-dictionary-Yoruba.jsonl"

_DIALECT_ID = 1


def download(raw_root: Path) -> Path:
    """Fetch the Yoruba kaikki JSONL and write raw_root/yoruba_dict/yoruba_dict.jsonl."""
    raw_root = Path(raw_root)
    raw_dir = raw_root / "yoruba_dict"
    raw_dir.mkdir(parents=True, exist_ok=True)
    resp = requests.get(_URL, timeout=120)
    resp.raise_for_status()
    out = raw_dir / "yoruba_dict.jsonl"
    out.write_text(resp.text, encoding="utf-8")
    n_lines = sum(1 for line in resp.text.splitlines() if line.strip())
    print(f"[yoruba_dict] wrote {n_lines} raw lines to {out}")
    return out
...
def transform(raw_root: Path, clean_dir: Path, cap: int | None = None) -> Path:
    ...
    if cap is None:
        cap = CAPS["yoruba_dict"]["total"]

    text = (raw_root / "yoruba_dict" / "yoruba_dict.jsonl").read_text(encoding="utf-8")
    ...
    out = clean_dir / "yoruba_dict_clean.csv"
    df.to_csv(out, index=False, encoding="utf-8", quoting=_csv.QUOTE_ALL)
    print(f"[yoruba_dict] wrote {len(df)} rows to {out}")
    return out
